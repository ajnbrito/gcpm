...
include_once "Cases.class.php"; 

class CasesDAO extends Cases 
{
    /** 
     * @access public 
     * @var Conexão 
 ...
 */ 
function insert() { 
    $stmt = $this->Conexao->pdo->prepare("INSERT INTO cases ( id_cases  ,  fk_id_usuario  ,  titulo  ,  texto_texto  ,  arquivo_arquivo ) VALUES (:id_cases  , :fk_id_usuario  , :titulo  , :texto_texto  , :arquivo_arquivo )"); 
        $stmt->BindParam(':id_cases', $this->getId_cases(),PDO::PARAM_INT, 11); 
 ...
 */ 
function delete() { 
    $stmt = $this->Conexao->pdo->prepare("DELETE FROM cases WHERE id_cases = :id_cases"); 
    $stmt->BindParam(':id_cases', $this->getId_cases(), PDO::PARAM_INT); 
    if ($stmt->execute()) { 
        return true; 
    } else { 
        return false; 
 ...
 */ 
function update() { 
    $stmt = $this->Conexao->pdo->prepare("UPDATE cases SET  id_cases = :id_cases  ,  fk_id_usuario = :fk_id_usuario  ,  titulo = :titulo  ,  texto_texto = :texto_texto  ,  arquivo_arquivo = :arquivo_arquivo  WHERE id_cases = :id_cases "); 
        $stmt->BindParam(':id_cases', $this->getId_cases(),PDO::PARAM_INT, 11); 
        $stmt->BindParam(':fk_id_usuario', $this->getFk_id_usuario(),PDO::PARAM_INT, 11); 
        $stmt->BindParam(':titulo', $this->getTitulo(),PDO::PARAM_STR, 255); 
        $stmt->BindParam(':texto_texto', $this->getTexto_texto(),PDO::PARAM_STR); 
 ...
 */ 
function listAll() { 
    $stmt = $this->Conexao->pdo->prepare("SELECT  id_cases  ,  fk_id_usuario  ,  titulo  ,  texto_texto  ,  arquivo_arquivo  FROM cases LIMIT " . $_SESSION['inicio'] . "," . $_SESSION['totalPorPg']); 
    $stmt->execute(); 
    $obj = $stmt->fetchAll(); 
    return $obj; 
} 
...
function listAllFull() { 
    $stmt = $this->Conexao->pdo->prepare("SELECT  id_cases  ,  fk_id_usuario  ,  titulo  ,  texto_texto  ,  arquivo_arquivo  FROM cases"); 
    $stmt->execute(); 
    $obj = $stmt->fetchAll(); 
    return $obj; 
} 
 ...
 */ 
function countAll() { 
    $stmt = $this->Conexao->pdo->prepare("SELECT COUNT(id_cases) as total FROM cases"); 
    $stmt->execute(); 
    $obj = $stmt->fetchAll(); 
    return $obj; 
} 
 ...
 */ 
function listPk() { 
    $stmt = $this->Conexao->pdo->prepare("SELECT  id_cases  ,  fk_id_usuario  ,  titulo  ,  texto_texto  ,  arquivo_arquivo  FROM cases WHERE id_cases = :id_cases"); 
    $stmt->BindParam(':id_cases', $this->getId_cases(), PDO::PARAM_INT); 
    $stmt->execute(); 
 ...
 */ 
function listFilter() { 
    $id_cases = '%' . $this->getId_cases() . '%'; 
     $fk_id_usuario = '%' . $this->getFk_id_usuario() . '%'; 
     $titulo = '%' . $this->getTitulo() . '%'; 
     $texto_texto = '%' . $this->getTexto_texto() . '%'; 
     $arquivo_arquivo = '%' . $this->getArquivo_arquivo() . '%'; 
     $sql = "SELECT   id_cases  ,  fk_id_usuario  ,  titulo  ,  texto_texto  ,  arquivo_arquivo "; 
    $sql .= " FROM cases WHERE 1=1 "; 
    if ($this->getId_cases()) { 
        $sql .= " AND id_cases LIKE :id_cases "; 
    } 
    ...
    if ($this->getId_cases()) { 
        $stmt->BindParam(':id_cases', $id_cases,PDO::PARAM_INT, 11); 
    } 
    if ($this->getFk_id_usuario()) { 
...
function combobox($selected) { 
    $obj = $this->listAll(); 
    echo "<select name='fk_cases' size='1' id='fk_cases'>"; 
...
function comboboxP($selected) { 
    $obj = $this->listAll(); 
    echo "<select name='fk_casesP' size='1' id='fk_casesP'>"; 
    echo "     <option value=''>Selecione...</option>"; 
    foreach ($obj as $campo) { 
        if ($campo[0] == $selected) { 
            echo "     <option value='" . $campo[0] . "' selected>" . $campo[1] . "</option>"; 
